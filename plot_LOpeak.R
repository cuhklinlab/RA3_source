...
# multi_donor = F;
if (multi_donor == T){
  pdf(file=sprintf("../results/plot/1021/Q3_scatter_multidonor%s.pdf",intv), width=(1+1)*plot_size, height=length(interval)*plot_size)
}else{
  pdf(file=sprintf("../results/plot/1021/Q3_scatter%s.pdf",intv), width=(1+1)*plot_size, height=length(interval)*plot_size)
}
tsne_perplexity <- 30
peakR <- 0

par(mfrow=c(length(interval),(1+1)), pty="s", mar=c(1, 3, 1, 1) + 0.1, cex.axis=1.5) 


H2type <- "MgOc"
for (exclude_peak_num in interval){
  for (data_name in c('forebrain_half')){
    # dataset_ind <- dataset_ind + 1
    for (DRmethod in c("RA3")){
      DRdim <- mainDRdim
      
      if (DRmethod=="RA3"){
        DRdim <- ourDRdim
        if ((multi_donor == T) & (data_name %in% c('ALL_blood','MPP_CMP_CLP','MPP_LMPP_CLP','all_forebrain','all_forebrain_RmEx'))){
          file_name <- sprintf('%s_%s_peak300_LOspePeak%s_top%d_peak%03d_dim%s_multidonor_Bulkforebrain_LO%s',DRmethod,data_name, H2type,exclude_peak_num, peakR*100,DRdim, H2type)
        }else{
          file_name <- sprintf('%s_%s_peak300_LOspePeak%s_top%d_peak%03d_dim%s_Bulkforebrain_LO%s',DRmethod,data_name, H2type,exclude_peak_num, peakR*100,DRdim, H2type)
        }
        ...
        DRdim <- mainDRdim
        file_name <- sprintf('%s_%s_peak300_LOspePeak%s_top%d_peak%03d_dim%s',DRmethod,data_name, H2type,exclude_peak_num, peakR*100,DRdim)
        label_file_name <- sprintf('/home/zhangwenyu/dataForComparison_clustering/202010/clusters/%s_clusters.tsv',file_name)

...
# multi_donor = F;
if (multi_donor == T){
  pdf(file=sprintf("../results/plot/1021/Q6_scatter_K3_multidonor%s.pdf",intv), width=length(interval)*plot_size, height=3*plot_size)
}else{
  pdf(file=sprintf("../results/plot/1021/Q6_scatter_K3%s.pdf",intv), width=length(interval)*plot_size, height=3*plot_size)
}
tsne_perplexity <- 30
...
for (K3 in interval){
      ...
      if (DRmethod=="RA3"){
        ...
        if ((multi_donor == T) & (data_name %in% c('ALL_blood','MPP_CMP_CLP','MPP_LMPP_CLP','all_forebrain','all_forebrain_RmEx'))){
          file_name <- sprintf('%s_%s_peak%03d_dim%s_multidonor_K3%d',DRmethod,data_name,peakR*100,DRdim, K3)
        }else{

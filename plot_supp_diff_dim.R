...
plot_scatter <- function(dims.reduce, cell.labels, data_type='NULL', cell.colors=NULL, donor.labels=NULL, multi_donor=F, perplexity=30,ourDRdim,data_name){
...
plot_size = 3.8
if (multi_donor == T){
  pdf(file=sprintf("../results/plot/0512/supp/scatter_multidonor_diff_dim_peak3_sub.pdf"), width=4*plot_size, height=5*plot_size)
}else{
  pdf(file=sprintf("../results/plot/0512/supp/scatter_diff_dim_peak3_sub.pdf"), width=4*plot_size, height=5*plot_size)
}
tsne_perplexity <- 30
...
par(mfrow=c(5,4), pty="s", mar=c(1, 3, 1, 1) + 0.1, cex.axis=1.5) 
# mar: margin between subplots c(bottom, left, top, right) 
# pty: square subplot

for (data_name in c('MPP_LMPP_CLP','donor_BM0828','ALL_blood','forebrain_half','tissue4_forebrain')){
  dataset_ind <- dataset_ind + 1
  for (ourDRdim in c(5,10,15)){
    ...
    if (DRmethod=="RA3"){
      ...
      label_file_name <- sprintf('/home/zhangwenyu/dataForComparison_clustering/clusters_our/%s_clusters.tsv',file_name)
      mat_file_name <- sprintf('~/data/resultForComparison/%s.mat',file_name)
      ...
      # DRres_mat <- t(DRres$H.hat)[,1:(DRres$K1[1]+DRres$K2[1])]
    }else{
      DRdim <- mainDRdim
      file_name <- sprintf('%s_%s_peak%03d_dim%s',DRmethod,data_name,peakR*100,DRdim)
      label_file_name <- sprintf('/home/zhangwenyu/dataForComparison_clustering/clusters/%s_clusters.tsv',file_name)
      rds_file_name <- sprintf('/home/zhangwenyu/dataForComparison_RDS/data_label_known/%s.rds',file_name)
      if (!file.exists(rds_file_name)) {
        print(sprintf('===[DOES NOT EXIST]=== %s', rds_file_name))

        file_name <- sprintf('%s_%s_peak%03d_dim%s','cisTopic',data_name,peakR*100,DRdim)
        label_file_name <- sprintf('/home/zhangwenyu/dataForComparison_clustering/clusters/%s_clusters.tsv',file_name)
    ...
    # if (data_name=='MPP_LMPP_CLP'){
    #   donor_file_name <- sprintf('~/data/resultForComparison/%s_donor_labels.mat',data_name)
    #   donor.label <- readMat(donor_file_name)$donor.label
    #   plot_scatter(DRres_mat, cluster_assign$label, data_type='blood', perplexity=tsne_perplexity, multi_donor=T, donor.labels=donor.label) 
    # }else # obvious donor effect
    if (dataset_ind < 4){
      plot_scatter(DRres_mat, cluster_assign$label, data_type='blood', perplexity=tsne_perplexity,ourDRdim=ourDRdim, data_name=data_name) # using color of source paper
    }else{
      plot_scatter(DRres_mat, cluster_assign$label, perplexity=tsne_perplexity,ourDRdim=ourDRdim, data_name=data_name) # using automatic color
    }
  }
}

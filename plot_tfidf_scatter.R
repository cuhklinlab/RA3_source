...
multi_donor = T;
if (multi_donor == T){
  pdf(file=sprintf("../results/plot/1021/Q4_scatter_multidonor.pdf"), width=4*plot_size, height=3*plot_size)
}else{
  pdf(file=sprintf("../results/plot/1021/Q4_scatter.pdf"), width=4*plot_size, height=3*plot_size)
}
tsne_perplexity <- 30
peakR <- 3

par(mfrow=c(3,4), pty="s", mar=c(1, 3, 1, 1) + 0.1, cex.axis=1.5) 
for (data_name in c('ALL_blood','forebrain_half','tissue4_forebrain')){
  for (DRmethod in c("PCA","PCAnorm","RA3")){
    DRdim <- mainDRdim
    if (DRmethod=="scABC"){
      next
    }
    ...
    if (DRmethod=="RA3"){
      DRdim <- ourDRdim
      if ((multi_donor == T) & (data_name %in% c('ALL_blood','MPP_CMP_CLP','MPP_LMPP_CLP','all_forebrain'))){
        file_name <- sprintf('Our_%s_peak%03d_dim%s_multidonor',data_name,100*peakR,DRdim);
      }else{
        file_name <- sprintf('Our_%s_peak%03d_dim%s',data_name,100*peakR,DRdim);
      }

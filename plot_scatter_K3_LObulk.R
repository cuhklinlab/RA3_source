...
# multi_donor = F;
if (multi_donor == T){
  pdf(file=sprintf("../results/plot/1021/Q6_scatter_K3_LObulk_multidonor%s.pdf",intv), width=length(interval)*plot_size, height=3*plot_size)
}else{
  pdf(file=sprintf("../results/plot/1021/Q6_scatter_K3_LObulk%s.pdf",intv), width=length(interval)*plot_size, height=3*plot_size)
}
tsne_perplexity <- 30
...
for (K3 in interval){
      ...
      if (DRmethod=="RA3"){
        ...
        if ((multi_donor == T) & (data_name %in% c('ALL_blood','MPP_CMP_CLP','MPP_LMPP_CLP','all_forebrain','all_forebrain_RmEx'))){
          file_name <- sprintf('%s_%s_peak%03d_dim%s_multidonor_Bulk%s_K3%d',DRmethod,data_name,peakR*100,DRdim, bulk_name,K3)
        }else{
          file_name <- sprintf('%s_%s_peak%03d_dim%s_Bulk%s_K3%d',DRmethod,data_name,peakR*100,DRdim, bulk_name,K3)
        }
        ...
        DRdim <- mainDRdim
        file_name <- sprintf('%s_%s_peak%03d_dim%s_K3%d',DRmethod,data_name,peakR*100,DRdim, K3)
        label_file_name <- sprintf('/home/zhangwenyu/dataForComparison_clustering/202010/clusters/%s_clusters.tsv',file_name)

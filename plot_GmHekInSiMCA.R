...
all_methods <- c("SCALE","Cusanovich2018","Scasat","scABC","SnapATAC","cisTopic","RA3")
...
# multi_donor = F;
if (multi_donor == T){
  pdf(file=sprintf("../results/plot/0512/supp/scatter_dim%s-%s_multidonor_GmHekInSiMCA.pdf", mainDRdim, ourDRdim), width=7*plot_size, height=6*plot_size)
}else{
  pdf(file=sprintf("../results/plot/0512/supp/scatter_dim%s-%s_GmHekInSiMCA.pdf", mainDRdim, ourDRdim), width=7*plot_size, height=6*plot_size)
}
tsne_perplexity <- 30
...
par(mfrow=c(6,7), pty="s", mar=c(1, 3, 1, 1) + 0.1, cex.axis=1.5) 
for (data_name in c('GMvsHL','InSilico','tissue1_forebrain','tissue2_forebrain','tissue3_forebrain')){

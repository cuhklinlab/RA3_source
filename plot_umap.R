...
plot_scatter <- function(dims.reduce, cell.labels, data_type='NULL', cell.colors=NULL, donor.labels=NULL, multi_donor=F, perplexity=30,DRmethod,data_name){
  ...
  if (data_name %in% c('ALL_blood','tissue4_forebrain','tissue2_forebrain','tissue3_forebrain')){
    plot(umap_DRres,    col=cells_colors_rgb, pch=donor_shape,xlab="",ylab="", cex = 0.6)#, main=file_name
  }
  else{
    plot(umap_DRres,    col=cells_colors_rgb, pch=donor_shape,xlab="",ylab="")#, main=file_name
  }
...
# multi_donor = F;
if (multi_donor == T){
  pdf(file=sprintf("../results/plot/0512/supp/scatter_dim%s-%s_multidonor_all_umap.pdf", mainDRdim, ourDRdim), width=7*plot_size, height=11*plot_size)
}else{
  pdf(file=sprintf("../results/plot/0512/supp/scatter_dim%s-%s_all_umap.pdf", mainDRdim, ourDRdim), width=7*plot_size, height=11*plot_size)
}
tsne_perplexity <- 30
...
par(mfrow=c(11,7), pty="s", mar=c(1, 3, 1, 1) + 0.1, cex.axis=1.5) 
for (data_name in c('MPP_LMPP_CLP','donor_BM0828','ALL_blood','GMvsHek','GMvsHL','InSilico','forebrain_half','tissue4_forebrain','tissue1_forebrain','tissue2_forebrain','tissue3_forebrain')){
  dataset_ind <- dataset_ind + 1
  for (DRmethod in c("scABC","SCALE","Scasat","cisTopic","Cusanovich2018","SnapATAC","RA3")){
    DRdim <- mainDRdim
    if (DRmethod=="scABC"){
      next
    }
    ...
    if (data_name=='MPP_LMPP_CLP'){
      ...
      plot_scatter(DRres_mat, cluster_assign$label, data_type='blood', perplexity=tsne_perplexity, multi_donor=T, donor.labels=donor.label,DRmethod=DRmethod,data_name=data_name) 
    }else if (dataset_ind < 4){

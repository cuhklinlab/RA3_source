...
# multi_donor = F;
if (multi_donor == T){
  pdf(file=sprintf("../results/plot/1021/Q2_scatter_multidonor.pdf"), width=(6+1)*plot_size, height=length(interval)*plot_size)
}else{
  pdf(file=sprintf("../results/plot/1021/Q2_scatter.pdf"), width=(6+1)*plot_size, height=length(interval)*plot_size)
}
tsne_perplexity <- 30
...
par(mfrow=c(length(interval),(6+1)), pty="s", mar=c(1, 3, 1, 1) + 0.1, cex.axis=1.5) 

H2type <- "MgOc"
for (H1drop_pct in interval){
  for (data_name in c('forebrain_half')){
    ...
    for (DRmethod in c("scABC","SCALE","Scasat","cisTopic","Cusanovich2018","SnapATAC","RA3")){
      ...
      if (DRmethod=="RA3"){
        ...
        if ((multi_donor == T) & (data_name %in% c('ALL_blood','MPP_CMP_CLP','MPP_LMPP_CLP','all_forebrain','all_forebrain_RmEx'))){
          file_name <- sprintf('%s_%s_H2%s_dropH1cell%03d_peak%03d_dim%s_multidonor_Bulkforebrain_LO%s',DRmethod,data_name, H2type,round(100*H1drop_pct), peakR*100,DRdim, H2type)
        }else{
          file_name <- sprintf('%s_%s_H2%s_dropH1cell%03d_peak%03d_dim%s_Bulkforebrain_LO%s',DRmethod,data_name, H2type,round(100*H1drop_pct), peakR*100,DRdim, H2type)
        }
        ...
        DRdim <- mainDRdim
        file_name <- sprintf('%s_%s_H2%s_dropH1cell%03d_peak%03d_dim%s',DRmethod,data_name, H2type,round(100*H1drop_pct), peakR*100,DRdim)
        label_file_name <- sprintf('/home/zhangwenyu/dataForComparison_clustering/202010/clusters/%s_clusters.tsv',file_name)

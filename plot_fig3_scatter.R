...
# multi_donor = F;
if (multi_donor == T){
  pdf(file=sprintf("../results/plot/0512/scatter_dim%s-%s_multidonor_fig3.pdf", mainDRdim, ourDRdim), width=7*plot_size, height=6*plot_size)
}else{
  pdf(file=sprintf("../results/plot/0512/scatter_dim%s-%s_fig3.pdf", mainDRdim, ourDRdim), width=7*plot_size, height=6*plot_size)
}
tsne_perplexity <- 30
...
for (data_name in c('MPP_LMPP_CLP','donor_BM0828','ALL_blood','forebrain_half','forebrain_half_do025','tissue4_forebrain')){

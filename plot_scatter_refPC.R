...
# multi_donor = F;
if (multi_donor == T){
  pdf(file=sprintf("../results/plot/1021/Q6_scatter_refPC_multidonor%s.pdf",intv), width=length(interval)*plot_size, height=2*plot_size)
}else{
  pdf(file=sprintf("../results/plot/1021/Q6_scatter_refPC%s.pdf",intv), width=length(interval)*plot_size, height=2*plot_size)
}
tsne_perplexity <- 30
...
par(mfrow=c(2,length(interval)), pty="s", mar=c(1, 3, 1, 1) + 0.1, cex.axis=1.5) 
bulks <- c("AllRO", "AllPO")
for (bulk_suffix in bulks){
  for (data_name in c('ALL_blood', 'GMvsHek')){
    if (data_name=='ALL_blood'){
      bulk = sprintf("blood_%s",bulk_suffix)
    }
    if (data_name=='GMvsHek'){
      bulk = sprintf("%s_%s",data_name,bulk_suffix)
    }

for (refPC in interval){
      DRmethod = "RA3"
      if (DRmethod=="RA3"){
        DRdim <- ourDRdim
        if ((multi_donor == T) & (data_name %in% c('ALL_blood','MPP_CMP_CLP','MPP_LMPP_CLP','all_forebrain','all_forebrain_RmEx'))){
          file_name <- sprintf('%s_%s_peak%03d_dim%s_multidonor_Bulk%s_refPC%d',DRmethod,data_name,peakR*100,DRdim, bulk,refPC)
        }else{
          file_name <- sprintf('%s_%s_peak%03d_dim%s_Bulk%s_refPC%d',DRmethod,data_name,peakR*100,DRdim, bulk,refPC)
        }
